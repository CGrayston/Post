...
class PostController {
    ...
    func fetchPosts(reset: Bool = true, newMessage: Bool = false, completion: @escaping() -> Void) {
    ...
    func addNewPostWith(username: String, text: String, completion: @escaping () -> Void) {
        ...
        do {
            ...
            print("Error encoding post \(error): \(error.localizedDescription)")
            completion()
            return
        }
        
        // Unwrap your base URL
        guard let unwrappedURL = baseURL else { completion(); return }
        ...
        let dataTask = URLSession.shared.dataTask(with: urlRequest) { (data, _, error) in
            ...
            self.fetchPosts(reset: false, newMessage: true) {
                completion()
            }
        }
        dataTask.resume()
